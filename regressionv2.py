...
# FILTRO DE OUTLIERS
# Remove os 5% de casos extremos que o modelo não tem como prever no "Dia 0"
limite_dias = df["dias_totais_internacao"].quantile(0.95)
df_limpo = df[df["dias_totais_internacao"] <= limite_dias]


# agrupa os cids com maior frequencia
top_15_cids = df_limpo["cid_principal"].value_counts().nlargest(15).index
df_limpo.loc[:, "cid_agrupado"] = df_limpo["cid_principal"].apply(
...
X = df_limpo[colunas_preditoras]
y = df_limpo["dias_totais_internacao"]

# função do próprio sklearn pra transformar variável categórica em numérica (OneHotEncoding)
...
print("Random forest")
# Limitando a profundidade da árvore (max_depth=10) para evitar que ela decore os dados
